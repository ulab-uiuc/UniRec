...
Place the raw input files where the scripts expect them (paths are currently hardcoded at the top of each script):

```
data_rec/
├── temp/
│   ├── meta_All_Beauty.jsonl     # item metadata  -> create_item_dict.py
│   └── All_Beauty.jsonl          # user reviews    -> create_review_dict.py
├── Amazon_All_Beauty.inter       # interactions    -> process_rec_*.py
...
- **`data_rec/temp/meta_All_Beauty.jsonl`** – item **metadata** (title, features, price, images, `parent_asin`, …).
- **`data_rec/temp/All_Beauty.jsonl`** – user **reviews** (rating, text, `user_id`, `parent_asin`, …). Only needed if you use reviews.
- **`data_rec/Amazon_All_Beauty.inter`** – a tab-separated interaction file. The first line is treated as a header and skipped; the loader reads the first four columns as `user_id`, `item_id`, `rating`, `timestamp`.

Only `data_rec/temp/` and `data_rec/Amazon_All_Beauty.inter` hold raw inputs. The `dict/`, `data/`, and `embeddings/` folders are populated by the scripts below.

Then run the dict builders and rec processors:

```bash
# Build item dictionary (reads data_rec/temp/meta_All_Beauty.jsonl)
python data_processing/create_item_dict.py

# Build review dictionary (reads data_rec/temp/All_Beauty.jsonl; only if using reviews)
python data_processing/create_review_dict.py

# Build triplet dictionary
python data_processing/create_triplet_dict.py

# Process recommendation data (reads data_rec/Amazon_All_Beauty.inter)
...
1. **Prepare data**
   - Place raw inputs where the scripts expect them: item metadata at `data_rec/temp/meta_All_Beauty.jsonl`, reviews at `data_rec/temp/All_Beauty.jsonl`, and the interaction file at `data_rec/Amazon_All_Beauty.inter`.
